 ...
 * 
 */
public class MyOrderActivity extends Activity implements IBaseActivity, OnClickListener, OnItemClickListener {
	...
	private ProfileMyOrdersTask profileMyOrdersTask;
	...
	private String from = null;// 自 收到的订单 或 我的订单跳转而来
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (NetUtil.checkNet(this)) {
			if (!LIST_RECORD_TASK_RUNING) {
				profileMyOrdersTask = new ProfileMyOrdersTask(userId, pageNo, pageSize, accessToken, state, state2, state3, state4, state5);
	...
	public void fillData() {
		if (pageNo == 1) {
			orderListAdapter = new OrderListAdapter(this, orderListBeans);
	...
	class OrderListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			case Constants.ROO_REFUSE:
				viewHolder.tvState.setText("袋鼠拒绝");
				break;
			case Constants.ROO_NOT_CONFIRM:
				viewHolder.tvState.setText("袋鼠未确认");
				break;
			case Constants.KOALA_NOT_PAY:
				viewHolder.tvState.setText("请支付");
				break;
			...
			case Constants.KOALA_NOT_CONFIRM:
				if (orderListBeans.get(position).getIsOnline() == ONLINE_PAY) {
					viewHolder.tvState.setText("请确认付款");
				} else {
					viewHolder.tvState.setText("线下支付");
				}
				break;
			...
			case Constants.ROO_NOT_COMMENT:
				viewHolder.tvState.setText("袋鼠未评价");
				break;
	 ...
	 * 
	 */
	class ProfileMyOrdersTask extends AsyncTask<Void, Void, JSONObject> {
		...
		public ProfileMyOrdersTask(long userId, int pageNum, int pageSize, String access_token, Integer state, Integer state2, Integer state3, Integer state4, Integer state5) {
			this.userId = userId;
		...
		protected void onPreExecute() {
			super.onPreExecute();
			if (pd == null) {
				pd = new ProgressDialog(MyOrderActivity.this);
				pd.setMessage("正在获取...");
			}
			pd.show();
		}
		...
		protected JSONObject doInBackground(Void... params) {
			try {
				return new BusinessHelper().profileMyOrders(userId, pageNum, pageSize, access_token, state, state2, state3, state4, state5);
			...
			if (result != null) {
				try {
					...
					if (status == Constants.SUCCESS) {
						...
						Toast.makeText(MyOrderActivity.this, R.string.time_out, Toast.LENGTH_LONG).show();
						startActivity(new Intent(MyOrderActivity.this, LoginActivity.class).putExtra("back", "back"));
					} else {
						tvShowNo.setVisibility(View.VISIBLE);
//						Toast.makeText(MyOrderActivity.this, result.getString("error"), Toast.LENGTH_LONG).show();
					}
				} catch (JSONException e) {
					Toast.makeText(MyOrderActivity.this, "获取失败", Toast.LENGTH_LONG).show();
				} catch (SystemException e) {
					e.printStackTrace();
					Toast.makeText(MyOrderActivity.this, "获取失败", Toast.LENGTH_LONG).show();
				}
			} else {
				Toast.makeText(MyOrderActivity.this, "服务器请求失败", Toast.LENGTH_LONG).show();
			}
			// 加载记录的任务结束
	...
	OnScrollListener loadNewPageListener = new OnScrollListener() {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			if (scrollState == OnScrollListener.SCROLL_STATE_IDLE && lastItem == orderListAdapter.getCount() - 1) {
				if (NetUtil.checkNet(MyOrderActivity.this)) {
					if (!LIST_RECORD_TASK_RUNING && pageNo < totalPage) {
						profileMyOrdersTask = new ProfileMyOrdersTask(userId, pageNo, pageSize, accessToken, state, state2, state3, state4, state5);
						profileMyOrdersTask.execute();
					}
				} else {
					Toast.makeText(MyOrderActivity.this, R.string.NoSignalException, Toast.LENGTH_LONG).show();
				}
			}
		}
	};
