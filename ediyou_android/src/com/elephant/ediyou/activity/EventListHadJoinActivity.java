 ...
 * 
 */
public class EventListHadJoinActivity extends Activity implements IBaseActivity, OnClickListener, OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (userId == (long) SharedPrefUtil.getUserBean(this).getUserId()) {
			isSelf = true;
		}
		findView();
		fillData();
		if (NetUtil.checkNet(this)) {
			new GetEventHadJoinListTask().execute();
		} else {
	...
	public void findView() {
		...
		if (isSelf) {
			tvTitle.setText("参加的活动");
		} else {
			tvTitle.setText(/*"考拉" + */"'" + name + "'" + "参加过的活动");
		}
	...
	OnScrollListener loadNewPageListener = new OnScrollListener() {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			if (scrollState == OnScrollListener.SCROLL_STATE_IDLE && lastItem == eventAdapter.getCount() - 1) {
				if (NetUtil.checkNet(EventListHadJoinActivity.this)) {
					if (!LIST_RECORD_TASK_RUNING) {
						if (pageNo < totalPage) {
							new GetEventHadJoinListTask().execute();
						} else {
							Toast.makeText(EventListHadJoinActivity.this, R.string.load_all, Toast.LENGTH_SHORT).show();
						}
					}
				} else {
					Toast.makeText(EventListHadJoinActivity.this, R.string.NoSignalException, Toast.LENGTH_SHORT).show();
				}
			}
...
//			TextView tvEventLag;
	 ...
	 * 
	 */
	class GetEventHadJoinListTask extends AsyncTask<Void, Void, JSONObject> {
		public GetEventHadJoinListTask() {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			if (pd == null) {
				pd = new ProgressDialog(EventListHadJoinActivity.this);
		...
		protected JSONObject doInBackground(Void... params) {
			try {
				return new BusinessHelper().currHadJoinEvent(pageNo, pageSize, userId);
					...
					if (status == Constants.SUCCESS) {
						...
						if (eventListJson.length() > 0) {
							...
							if (eventAdapter == null || eventAdapter.getCount() == 0) {
								eventAdapter = new EventAdapter(EventListHadJoinActivity.this);
							...
							Toast.makeText(EventListHadJoinActivity.this, "您还没有参加过活动哦", Toast.LENGTH_SHORT).show();
						}
					} else if (status == Constants.TOKEN_FAILED) {
						Toast.makeText(EventListHadJoinActivity.this, R.string.time_out, Toast.LENGTH_SHORT).show();
						startActivity(new Intent(EventListHadJoinActivity.this, LoginActivity.class).putExtra("back", "back"));
					} else {
						tvShowNo.setVisibility(View.VISIBLE);
//						Toast.makeText(EventListHadJoinActivity.this, result.getString("error"), Toast.LENGTH_SHORT).show();
					}
				} catch (JSONException e) {
					Toast.makeText(EventListHadJoinActivity.this, "加载失败", Toast.LENGTH_SHORT).show();
				}
			} else {
				Toast.makeText(EventListHadJoinActivity.this, "服务器请求失败", Toast.LENGTH_SHORT).show();
			}
			// 加载记录的任务结束
