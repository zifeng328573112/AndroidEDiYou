...
public class ImageCacheLoader {
	...
	public static final int IMAGE_QUALITY = 70;//图片质量
	
	// 为了加快速度，在内存中开启缓存（主要应用于重复图片较多时，或者同一个图片要多次被访问，比如在ListView时来回滚动）
	public Map<String, Drawable> imageCache = new HashMap<String, Drawable>();
	
	private static ExecutorService executorService; // 固定10个线程来执行任务
	private final Handler handler = new Handler();
	private static ImageCacheLoader instance;
	 ...
	 */
	public void clear(){
		for (Entry<String, Drawable> entry : imageCache.entrySet()) {       
	 ...
	 * @param imageCache
	 */
	public void clear(Map<String, Drawable> imageCache){
//		try{
//			for (Entry<String, Drawable> entry : imageCache.entrySet()) {       
//				Drawable drawable = entry.getValue();
//				if(drawable!=null)
...
//		}catch(Exception e){}
	 ...
	 * @return
	 */
	public Drawable loadSoftDrawable(final Map<String, Drawable> imageCache,int maxSize,final String imageUrl,final ImageCallback callback) {
		// 如果缓存过就从缓存中取出数据
	...
	private static final File noMedia =  new File(Environment.getExternalStorageDirectory()+ "/"+Constants.APP_DIR_NAME+"/",".nomedia"); 
...
//			        } 
			        ...
			        if(bm!=null){
			            ...
			            bm.compress(Bitmap.CompressFormat.JPEG, IMAGE_QUALITY, outStream);  
			            outStream.flush();  
			            outStream.close();
			        }
					is.close();
...
//			        bm = BitmapFactory.decodeStream(is);
			        if(bm!=null){
						...
						if(IMAGE_TYPE_OF_PNG.equals(imgType)){
							bm.compress(Bitmap.CompressFormat.PNG, IMAGE_QUALITY, outStream);  
						}
